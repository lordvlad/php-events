...
trait EventEmitterTrait {
  ...
  public function addListener($event, callable $listener) {
    if (!is_string($event))
      throw new InvalidArgumentException(sprintf(EventEmitter::NOTASTRING, __METHOD__, gettype($event)));

    if (array_reduce($this->_listeners, function($c, $i){return $c += count($i);}, 0) > $this->_maxListeners)
      throw new RuntimeException(sprintf(EventEmitter::TOOMANY, $this->_maxListeners));
  ...
  public function setMaxListeners($n) {
    if (!is_int($n))
      throw new InvalidArgumentException(sprintf(EventEmitter::NOTANINT, __METHOD__, gettype($n)));
